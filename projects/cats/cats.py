...
def pick(paragraphs, select, k):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    p = [x for x in paragraphs if select(x)]
...
def about(subject):
    ...
    def _about(ps):
        flag = False
        for i in subject:
            if i in split(lower(remove_punctuation(ps))):
                flag = True
        return flag
    return _about
    # END PROBLEM 2
...
def autocorrect(typed_word, word_list, diff_function, limit):
    ...
    diff_value = [diff_function(typed_word, word, limit) for word in word_list]
    min_diff_value = min(diff_value, key=abs)

    if min_diff_value > limit:
        return typed_word

    for word in word_list:
        if min_diff_value == diff_function(typed_word, word, limit):
            return word
    # END PROBLEM 5
...
def time_per_word(words, timestamps_per_player):
    ...
    for i in range(len(timestamps_per_player)):
        ...
        for j in range(len(timestamps_per_player[i]) - 1):
            prev = timestamps_per_player[i][j]
            next = timestamps_per_player[i][j + 1]

            player_time += [next - prev]
        time += [player_time]
...
def fastest_words(match):
    ...
    for word_index in word_indices:
        faster_player_index = 0

        for player_index in player_indices:
            if time(match, player_index, word_index) < time(match, faster_player_index, word_index):
                faster_player_index = player_index

        results[faster_player_index] += [get_word(match, word_index)]
    return results
    # END PROBLEM 10
